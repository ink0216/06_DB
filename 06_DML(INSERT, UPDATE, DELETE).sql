...
INSERT INTO EMPLOYEE2 VALUES(900, '장채현', '901123-2345678', 'jang_ch@kh.or.kr', '01012341234',
'D1', 'J7', 'S3', 4300000, 0.2, 200, SYSDATE, NULL, 'N');
--14개의 컬럼 값을 순서대로 작성해야 삽입된다.
SELECT * FROM EMPLOYEE2
WHERE EMP_ID =900;
...
ROLLBACK; 
--ROLLBACK : 우리가 INSERT한 내용들이 다 사라짐(장채현, 개발사업부)
-----------------------------------------------------------------------------------------------
-- 2)  INSERT INTO 테이블명(컬럼명, 컬럼명, 컬럼명,...)
-- VALUES (데이터1, 데이터2, 데이터3, ...);
-- 테이블에 내가 선택한 컬럼에 대한 값만 INSERT할 때 사용
-- 선택안된 컬럼은 값이 NULL이 들어감
-- 전체 컬럼에 넣는 것이 아닌, 선택된 컬럼에만 넣고 싶을 때!
-- 선택 안된 컬럼에는 NULL이 들어감!!
...
--이 컬럼들에 값 넣어야지~~할 때 소괄호 내의 컬럼의 순서는 아무렇게나 해도 괜찮은데, 
--VALUES에 넣는 값은 작성한 컬럼 순서대로 VALUES의 값을 작성해야 한다!!
SELECT * FROM EMPLOYEE2
WHERE EMP_ID =900;

INSERT INTO DEPARTMENT2 (DEPT_ID, LOCATION_ID)
VALUES ('D0','L1');
SELECT * FROM DEPARTMENT2; --TITLE은 삽입 안했어서 NULL이 들어감
ROLLBACK;
--------------------------------------------------------------------------------------------------
...
INSERT INTO EMP_01 (
	...
	--서브쿼리의 결과 행이 모두 삽입됨!(23행 삽입됨)
);
SELECT * FROM EMP_01;
COMMIT;
--------------------------------------------------------------------------------------------------------------------
-- 2. UPDATE : 수정
-- UPDATE == 기존 프로그램의 코드만 유지보수하는 것
-- UPGRADE == 새 프로그램 만드는 것

-- 테이블에 기록된 컬럼의 값을 수정하는 구문

-- [작성법]
-- UPDATE 테이블명 SET 컬럼명 = 바꿀값 [WHERE 컬럼명 비교연산자 비교값]; --조건이 들어갈 수 있다
...
--총무부를 전략기획팀으로 수정
UPDATE DEPARTMENT2 
SET DEPT_TITLE = '전략기획팀' --이 값을 세팅할거야
WHERE DEPT_ID ='D9'; --여기에
--1행 수정됨
-- UPDATE 확인
SELECT *
FROM DEPARTMENT2
...
----------------------------------------------------------------------------------
-- * 조건절을 설정하지 않고 UPDATE 구문 실행 시 모든 행의 컬럼 값이 변경된다.
UPDATE DEPARTMENT2 
SET DEPT_TITLE ='인사팀';

SELECT * FROM DEPARTMENT2; --WHERE절 안쓰면 모든 행의 DEPT_TITLE을 인사팀으로 바꿈
...
UPDATE EMPLOYEE2 
SET BONUS=0.3
WHERE EMP_ID IN ( --다중행이므로, =이 아닌 IN으로 비교해야함!!!!!!!!!
	SELECT EMP_ID
	FROM EMPLOYEE2 
	JOIN DEPARTMENT2 ON (DEPT_ID=DEPT_CODE)
	JOIN LOCATION ON (LOCATION_ID=LOCAL_CODE)
...
AS SELECT * FROM EMPLOYEE; --복사해서 만듦

CREATE TABLE EMP_M02
AS SELECT * FROM EMPLOYEE
   WHERE JOB_CODE = 'J4'; --J4인 사람들만 복사해서 만듦
   
INSERT INTO EMP_M02 --새 사람 추가
...
UPDATE EMP_M02 SET SALARY = 0; --모두의 급여를 0원으로 만들기
...
WHEN MATCHED THEN
UPDATE SET
--EMP_M01 안에다가 합칠거다 합칠 때 사용할 테이블은 EMP_M02테이블 둘 다 사번 컬럼을 이용해서
--만약에 둘의 사번이 일치하면(MATCHED) UPDATE를 하겠다
EMP_M01.EMP_NAME = EMP_M02.EMP_NAME, --2번 테이블의 값을 1번 테이블로 대입 (오른쪽 값을 왼쪽에 대입)
...
WHEN NOT MATCHED THEN --일치되지 않으면 INSERT를 하겠다
...
COMMIT;--이전 것 안지워지도록 저장하기

-- EMPLOYEE2 테이블에서 '장채현'사원 정보 조회
SELECT * FROM EMPLOYEE2
WHERE EMP_NAME ='장채현'; --확인해서 장채현 없으면 위에 만드는 INSERT 구문 실행하면 생김
...
-- EMPLOYEE2 테이블 전체 삭제 : WHERE절 안쓰면 전체가 삭제됨!!(UPDATE 구문과 똑같음)
DELETE FROM EMPLOYEE2;--23행 삭제
SELECT * FROM EMPLOYEE2;
ROLLBACK;
---------------------------------------------------------------------------------------------
-- 5. TRUNCATE (DDL 입니다! DML 아닙니다!) : 잘라버리기(버림)
-- 테이블의 전체 행을 삭제하는 DDL
